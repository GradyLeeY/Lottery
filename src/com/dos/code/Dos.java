...
class User{
    ...
    public void getLuckly() {
        ...
        while(lucklyUsers.size()<5)
        {
            ...
            while((u=(new operatorFile(id=String.valueOf (i))).getUserMess(1)).size()<1){ }
...
class operatorFile {
    ...
    public ArrayList<String> getUserMess(int index){
        ...
        File file=new File("D:\\interest\\dailyCode\\src\\com\\dos\\doc\\user.text");
        String line="";
        try{
    ...
    public void writeUserMess(User u){
        try{
            BufferedWriter bw=new BufferedWriter(new FileWriter(new File("D:\\interest\\dailyCode\\src\\com\\dos\\doc\\user.text"),true));
            bw.write(u.toString()+"\n");
            bw.close();
        }
        catch (IOException e){ }
